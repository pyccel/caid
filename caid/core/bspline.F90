...
subroutine EvaluateDeriv(d,n,p,U,Pw,r,X,Cw)
  ...
  integer(kind=4), intent(in)  :: r
  real   (kind=8), intent(in)  :: X(0:r)
  real   (kind=8), intent(out) :: Cw(d,0:r,0:1)
  integer(kind=4) :: i, j, span, deriv
  real   (kind=8) :: dbasis(0:p,0:1), C(d), w(0:1)
  real   (kind=8) :: Rdbasis(0:p,0:1)
  real   (kind=8) :: basis(0:p)
  !
  do i = 0, r
     span = FindSpan(n,p,X(i),U)
     call DersBasisFuns(span,X(i),p,1,U,dbasis)
     ...
     w  = 0.0
     do j = 0, p
        w  = w  + dbasis(j,:)*Pw(d,span-p+j)
     end do
     ! compute Nurbs
     Rdbasis  = 0.0
     do j = 0, p
        Rdbasis(j,0) = dbasis(j,0) / w(0)
        Rdbasis(j,1) = dbasis(j,1) / w(0) - dbasis(j,0) * w(1) / w(0)**2 
     end do

     do deriv = 0, 1 
        C = 0.0
        do j = 0, p
           C = C + Rdbasis(j,deriv)*Pw(:,span-p+j)
        end do
        Cw(:,i, deriv) = C
     end do
     !
